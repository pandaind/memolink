...
1. [ORIENTATION] BEFORE writing any code or answering the user, you MUST use the 'search_md_files' MCP tool to find guidelines or past architectural decisions related to: "$USER_PROMPT"
2. [EVALUATION] Review the 'score' of the returned search results. Ignore results with a score below 0.3.
3. [DEEP DIVE] Use 'get_graph_context' on the most relevant file ID. This provides both the file content AND its immediate neighbors in the graph, giving you maximum context with minimal tool calls.
4. [EXECUTION] Only after reading the vault context should you fulfill the user's request.
5. [PERSISTENCE] When the task is complete, you MUST use the 'create_md_file' tool to write a detailed summary log of what you changed. Tag it with #agent-log and #${CURRENT_BRANCH}.
